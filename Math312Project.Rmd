...
## The following function will perform Heun's Method on a system
## with 2 dependent variables (RHS depends on x, y, and t)
## the functions needs to be specified ahead of time as 
## Fxyt<-function(x,y,t){f}    where f is some function of x, y and t
## Gxyt<-function(x,y,t){g}    where g is some function of x, y and t
## e.g. for f(x,y,t) = x + y + sin(t)
## Fxyt<-function(x,y,t){x + y + sin(t)}
## Heun's Method starts at t=a and runs to t=b
## where (x(a),y(a)) = (x0,y0) and the step size is delta(t) = (b-a)/(n-1)
## Usage:  after Fxyt and Gxyt have been specified run the command
## HeunTable<-HeunSystemNATable(Fxyt,Gxyt,x0,y0,a,b,n)
## with appropriate values of x0, y0, a, b, and n
## to see the table of approximate values of y run
## HeunTable in the console

## Automatically plots y(x), but you can plot x(t) using plot(HeunTable$t, HeunTable$x)
## Can see both plots at the same time if you do plot() for one and point()
## add t axis with points(HeunTable$t, 0*HeunTable$x)
...
server_KNN <- function(input, output) {
  output$model_plot <- renderPlot({
    HeunSystemNATable(Fxyt, Gxyt, input$x0_pick, input$y0_pick, input$a_pick, input$b_pick,  (input$b_pick-input$a_pick) * 100 + 1) #currently automatically using delta t = 0.01
  })
}
...
server_KNN <- function(input, output) {
  output$model_plot <- renderPlot({
    EulerNATable(Fyt, input$y0_pick, input$a_pick, input$b_pick,  (input$b_pick-input$a_pick) * 100 + 1)
...
ui_KNN <- fluidPage(
  sidebarLayout(
    sidebarPanel(
      ...
      numericInput("y0_pick", "Initial y value (y0)", value = 0, min = 0, max = 100),
      numericInput("a_pick", "Start Time", value = 0, min = 0, max = 100),
      numericInput("b_pick", "End Time", value = 0, min = 0, max = 100),
